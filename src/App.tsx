...
import { configureLogger } from '@0xsodium/utils'
import { Group } from './components/Group'
import { Button } from './components/Button'
import { Console } from './components/Console'
import { sodiumConnection } from './connectors/index'

configureLogger({ logLevel: 'DEBUG' })

const App = () => {
  const { connector, hooks, type } = sodiumConnection;
  ...
  And that has made all the difference.`
      ...
      const isValid = await sodium.utils.isValidMessageSignature(await signer.getAddress(), message, sig, provider!)
  ...
  const signTypedData = async () => {
    try {
      ...
      const isValid = await sodium.utils.isValidTypedDataSignature(await signer.getAddress(), typedData, sig, provider!)
      console.log('isValid?', isValid)
      appendConsoleLine(`isValid? ${isValid}`)
      setConsoleLoading(false) 
    } catch(e) {
